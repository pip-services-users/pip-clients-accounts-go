...
func fromAccountPage(page *data.DataPage) *protos.AccountPage {
	if page == nil {
		return nil
	}
...
func toAccountPage(obj *protos.AccountPage) *data.DataPage {
	if obj == nil {
		return nil
	}

	accounts := make([]interface{}, len(obj.Data))

	for i, v := range obj.Data {
		accounts[i] = toAccount(v)
	}

	page := data.NewDataPage(&obj.Total, accounts)
